...
export const product = (cb: IHttpHook['request']) => ({
 ...
 getProducts: ({
  ...
  cb(`products?offset=${offset}&limit=15${search ? `&search=${search}` : ''}`),
 getProductsBySubCategory: ({
  ...
  cb(
   `products/${category}/${subCategory}?offset=${offset}&limit=15${
 ...
 getProductsByCategory: ({
  ...
  cb(`products/${category}?offset=${offset}&limit=15${search ? `&search=${search}` : ''}`),
 searchProduct: ({
  value,
  category,
  offset,
